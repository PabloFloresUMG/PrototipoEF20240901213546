      ...
      // logica logicaSeg = new logica();
        ...
        private void CargarDeducciones()
        {
            try
            {
                ...
                {
                    Dgv_gendeducciones.DataSource = deducciones;
                    logicaSeg.funinsertarabitacora(idUsuario, "Se consulto deducciones y se encontraron datos", "tbl_dedu_perp_emp", "6005");
                }
                else
                {
                    MessageBox.Show("No se encontraron deducciones.");
                    logicaSeg.funinsertarabitacora(idUsuario, "Se consulto deducciones y no se encontraron datos", "tbl_dedu_perp_emp", "6005");
                }
            }
            catch (Exception ex)
            {
        ...
        private void Btn_ayuda_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              //  MessageBox.Show("Ruta de búsqueda: " + ayudaFolderPath);
                ...
                string pathAyuda = FindFileInDirectory(ayudaFolderPath, "AyudaDeducciones.chm");

                if (!string.IsNullOrEmpty(pathAyuda))
                {
                    Help.ShowHelp(null, pathAyuda, "ayudaDeducciones_2024.html");
                }
                else
                {
